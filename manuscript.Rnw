...
plot(
  ...
  scale_colour_manual(
    "Ethnocentrism", limits = c("high", "low"), values = c("Red", "Blue")
  ) +
  facet_grid(
    rows = vars(feature), 
    cols = vars(dataset), 
...
<<hainmueller_mm_diffs, dependson=c("data_hainmueller_immigration"), fig.height = 5, fig.width=8, fig.cap="Differences in Conditional Marginal Means, by Ethnocentrism, for Hainmueller et al.'s (2014) Immigration Experiment", message=FALSE>>=
